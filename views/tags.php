...
<?php
    $keytag = "%$tag%";
    $stm = $_DB->prepare("SELECT articles.atid,articles.title,articles.content,articles.poston,subjects.subject_title,users.full_name FROM articles JOIN subjects ON articles.subject = subjects.subject_id JOIN users ON articles.uid = users.uid WHERE articles.public = 1 AND articles.tags LIKE :tag ORDER BY articles.atid DESC LIMIT 20");
    $stm->bindParam(":tag", $keytag);
    $stm->execute();
